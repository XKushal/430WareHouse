...
public class UserInterface {
  ...
  public void addProduct() {
    ...
    do {
	  ...
	  int productPrice = getNumber("Enter the products price");
      result = warehouse.addProduct(productName, productId, productQuantity, productPrice);
      if (result != null) {
        System.out.println(result);
      } else {
  ...
  public void acceptCustomerOrder() {
	...
	String invoiceId = warehouse.createInvoice();
	System.out.println("Your new invoice Id is: " + invoiceId);
	do {
	  i++;
	  String productId = getToken("Enter product id: ");
      int p_quantity = getNumber("Please Enter the product quantity: ");
	  if (warehouse.searchProduct(productId) == null) {
		System.out.println("No such Product");
		return;
	  }
	  result = warehouse.acceptCustomerOrder(clientId, invoiceId, productId, p_quantity, i);
	  if (result != null) {
	     System.out.println("Product accepted");
	  } else {
	     System.out.println("Product could not be accepted");
	  }
	  if (!yesOrNo("More Products on order? ")){
	     break;
	  }
	} while (true);
  }
  public void processCustomerOrder() {
	Invoice result;
	String invoiceId = getToken("Enter invoice id: ");
	if(warehouse.searchInvoice(invoiceId) == null) {
		System.out.println("No such invoice");
		return;
	}
	do{
		result = warehouse.processCustomerOrder(invoiceId);
		if (result != null) {
			System.out.println("Processed Customers order, Thank you");
		} else {
			System.out.println("Could not process Customers order.");
		}
		if(!yesOrNo("Process another order? ")){
			break;
		}
	} while (true);
  }
  ...
  public void generateInvoice() {
      String invoiceId = getToken("Enter invoice ID to search for: ");
      ...
      Invoice invoiceResult = warehouse.searchInvoice(invoiceId);
	  for(int i = 1; i<=invoiceResult.getI(); i++) {
        System.out.println(invoiceResult.toString(i));
      }
  }
  public void acceptCustomerPayment() {
        ...
        do {
		  int balance  = warehouse.getBalance(clientId);
		  System.out.println("Your current balance is $" + balance);
          int payment = getNumber("Please Enter the amount you would like to pay: ");
          result = warehouse.acceptCustomerPayment(clientId, payment);
          if (result != null) {
             System.out.println("Payment accepted");
          } else {
             System.out.println("Payment could not be accepted");
          }
          if (!yesOrNo("Would you like to make more payments? ")){
             break;
          }
        } while (true);
  }
